...
select c.name
from customers c
where concat(c.customer_id, c.name) in 
(select concat(customers.customer_id, customers.name)  
from customers where city = 'Kathmandu')


--- multiple column / table 

select  sum(total_amt)
from (select category, sum(price) total_amt from products group by 1 )
...
SELECT 
    c.city,
    o.status,
    CASE WHEN o.status = 'Delivered' THEN 'D '
     WHEN o.status = 'Shipped' then 'S'
    when o.status = 'Processing' THEN 'P' else 'PD' end AS Short_status
FROM customers c
JOIN orders o ON c.customer_id = o.customer_id
-- GROUP BY c.city;
